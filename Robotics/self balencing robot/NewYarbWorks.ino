...
// CONSTS
  //hadi depends of the wait distribution f robot
#define ACC_CALIBRATION_VALUE -8450                       
  // PID 
float Kp = 10 ;//10
float Ki = 1; // was 1.5
float Kd = 30 ;//30
...
void recieveData(){
  ...
  if(receive_counter <= 25)receive_counter ++; //The received byte will be valid for 25 program loops (100ms)
  else received_byte = '\0'; //After 100 ms the received byte is deleted
}
...
void calculatePID(){
  ...
  if(pid_output > 10 || pid_output < -10) pid_error += pid_output * 0.015 ; // this will resiste change when pushed by hand
...
void handleVirbation(int err = 5){
  ...
  if(pid_output < err && pid_output > -err)pid_output = 0;
}
...
void calculateControls(float setTilte = 2.5){
  ...
  if(received_byte == 'a'){
    ...
    if(pid_setpoint > -setTilte)pid_setpoint -= 0.05;                            
...
void handleTiltedSurfaces(){
  ...
  if(pid_setpoint == 0){                                                    //If the setpoint is zero degrees
    if(pid_output < 0)self_balance_pid_setpoint += 0.015;                  //Increase the self_balance_pid_setpoint if the robot is still moving forewards
    if(pid_output > 0)self_balance_pid_setpoint -= 0.015;                  //Decrease the self_balance_pid_setpoint if the robot is still moving backwards
  }
}
...
void loop(){
  ...
  throttle_left_motor = convertToLinear(pid_output_right);
